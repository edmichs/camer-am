...
class CreateAssureTable extends Migration {
	...
	public function up()
	{
		Schema::create('assure', function(Blueprint $table)
		{
			...
			$table->integer('RemboursementsID')->nullable();
			...
			$table->float('Taux_couverture', 10, 0)->default(0);
			$table->float('Plafond', 10, 0)->default(0);
			$table->float('Encour_conso', 10, 0)->default(0);
			$table->float('Solde', 10, 0)->default(0);
			$table->string('Nationalite')->nullable();
			$table->date('Date_incorporation')->nullable();
			$table->string('Discriminator')->nullable();
			$table->integer('AssureID')->nullable();
